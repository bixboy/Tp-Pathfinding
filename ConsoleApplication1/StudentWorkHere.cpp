...
void Game::GenerateMazeUsingDFS() 
{
    ...
    std::vector<std::vector<bool>> visited(static_cast<size_t>(height), std::vector<bool>(static_cast<size_t>(width), false));
...
void PlayerController::ComputePathUsingAStar(std::shared_ptr<Unit> unit, CVect to)
{
        ...
        std::vector<std::vector<unsigned int>> gScore(static_cast<size_t>(height), std::vector<unsigned int>(static_cast<size_t>(width), kInfinity));
        std::vector<std::vector<bool>> closed(static_cast<size_t>(height), std::vector<bool>(static_cast<size_t>(width), false));
        struct Parent
        {
                int x = -1;
                int y = -1;
        };
        std::vector<std::vector<Parent>> parents(static_cast<size_t>(height), std::vector<Parent>(static_cast<size_t>(width)));
        ...
        if (found)
        {
                ...
                if (distanceToGoal == kInfinity)
                        distanceToGoal = 0;
                reversePath.reserve(static_cast<size_t>(distanceToGoal) + 1u);
